...
public class BanCommand extends Command {
    ...
    public BanCommand(SlashCommandInteractionEvent event) {
        super(event);
    }
    ...
    public void execute() {
        ...
        User target = Database.getUser(Field.MINECRAFT_UUID, uuid).setBanned(true);
        if (!target.equals(User.EMPTY)) {
            Database.addUser(target);
            event.replyEmbeds(Embeds.profileEmbed(target)).setEphemeral(true).queue();
        } else {
            event.reply("Could not find player with name " + name).setEphemeral(true).queue();
        }
    }
